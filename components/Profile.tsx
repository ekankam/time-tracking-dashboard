...
const Tab = ({ className, name, onFilter }: TabProps) => {
  return (
    <li className={cn("font-normal text-18 text-center w-full lg:text-start cursor-pointer hover:text-white", className)} onClick={onFilter}>{name}</li>
  );
}

export default function Profile({ className, onFilter, timeframe }: ProfileProps) {
  const isActive = (value: string) => timeframe === value ? 'text-white' : 'text-c-dark-blue-300';
  return (
    <aside className={cn("rounded-c-radius bg-c-dark-blue-500", className)}>
      ...
      <ul className="flex items-center py-6 lg:px-8 lg:pt-[26px] lg:pb-8 lg:flex-col lg:gap-[21px]">
        <Tab className={isActive('daily')} name="Daily" onFilter={() => onFilter('daily')} />
        <Tab className={isActive('weekly')} name="Weekly" onFilter={() => onFilter('weekly')} />
        <Tab className={isActive('monthly')} name="Monthly" onFilter={() => onFilter('monthly')} />
      </ul>
    </aside>
  )
}